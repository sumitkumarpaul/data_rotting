...
int32_t test_inner_product_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl,
	 ...
	 */
#define USE_32_BIT_RESULT 1 /* Sumit */
#if USE_32_BIT_RESULT
	uint32_t x, y;
	 ...
	 */
	for (i = 0; i < numbers; i++) {
#if 0/* Instead of random numbers, compute 1*1 + 2*2 + 3*3 + ... + n*n */
		x = rand();
		y = rand();
#else
		x = (i+1);
...
#if USE_32_BIT_RESULT
	s_x_vec = circ->PutSIMDINGate(numbers, xvals.data(), 32, SERVER);
	s_y_vec = circ->PutSIMDINGate(numbers, yvals.data(), 32, CLIENT);
#else
